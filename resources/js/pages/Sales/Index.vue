...
const sortField = ref(props.filters.sort ?? 'sale_number');
...
function exportExcel() {
    ...
    window.location.href = `${SaleController.exportMethod.url()}?${params}`;
}

function deleteSale(id: number, saleNumber: string) {
    if (confirm(`売上「${saleNumber}」を削除してもよろしいですか？`)) {
        router.delete(SaleController.destroy.url(id));
    }
}
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <div class="flex flex-col gap-4 p-4">
            ...
            <div class="flex items-center justify-between">
                <h1 class="text-2xl font-bold">売上</h1>
            ...
            >
                <span>
                    全
                    <strong class="text-foreground">{{ sales.total }}</strong>
                    件
                    <template v-if="sales.from && sales.to"
                        >（{{ sales.from }}〜{{ sales.to }} 件表示）</template
                    >
                </span>
            ...
            <div class="overflow-x-auto rounded-md border">
                <table class="w-full text-sm">
                    ...
                    <tbody>
                        ...
                        >
                            <td
                                v-if="columns.sale_number.visible"
                            ...
                            <td
                                v-if="columns.sale_date.visible"
