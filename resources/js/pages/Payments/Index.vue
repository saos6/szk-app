...
const sortField = ref(props.filters.sort ?? 'payment_number');
...
function exportExcel() {
    ...
    window.location.href = `${PaymentController.exportMethod.url()}?${params}`;
}

function deletePayment(id: number, paymentNumber: string) {
    if (confirm(`入金「${paymentNumber}」を削除してもよろしいですか？`)) {
        router.delete(PaymentController.destroy.url(id));
    }
}
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <div class="flex flex-col gap-4 p-4">
            ...
            <div class="flex items-center justify-between">
                <h1 class="text-2xl font-bold">入金</h1>
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                >
                    ...
                    <SelectContent>
                        ...
                        <SelectItem v-for="(label, key) in statuses" :key="key" :value="key">
                            {{ label }}
                        </SelectItem>
                    </SelectContent>
                </Select>
                <Button variant="secondary" size="sm" @click="handleSearch">検索</Button>
                <Button variant="ghost" size="sm" @click="() => { search = ''; status = ''; handleSearch(); }">
                    クリア
                </Button>
            </div>
            ...
            <div class="flex items-center justify-between text-sm text-muted-foreground">
                <span>
                    全 <strong class="text-foreground">{{ payments.total }}</strong> 件
                    <template v-if="payments.from && payments.to">
                        （{{ payments.from }}〜{{ payments.to }} 件表示）
                    </template>
                </span>
            ...
            <div class="overflow-x-auto rounded-md border">
                <table class="w-full text-sm">
                    ...
                    <tbody>
                        ...
                        >
                            <td v-if="columns.payment_number.visible" class="px-4 py-3 font-mono text-sm">
                            ...
                            <td v-if="columns.payment_date.visible" class="px-4 py-3 whitespace-nowrap text-muted-foreground">
                            ...
                            <td v-if="columns.updated_at.visible" class="px-4 py-3 whitespace-nowrap text-muted-foreground">
                                {{ payment.updated_at ? new Date(payment.updated_at).toLocaleString('ja-JP') : '—' }}
                            </td>
                            <td class="px-4 py-3">
                                <div class="flex gap-1">
                                    <Button variant="ghost" size="icon" class="h-8 w-8" title="参照" as-child>
                                        <Link :href="PaymentController.show.url(payment.id)">
                                            <Eye class="h-4 w-4" />
                                        </Link>
                                    </Button>
                                    ...
                                    <Button
                                        ...
                                        title="削除"
                                        @click="deletePayment(payment.id, payment.payment_number)"
                                    >
