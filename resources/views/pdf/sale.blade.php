...
<style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .sale-number { font-size: 13px; font-weight: bold; }
    /* ─── 合計金額 ─── */
    .total-box {
...
<tr>
    <!-- 宛先 -->
    <td class="to-block">
        <div class="customer-name">{{ $sale->customer?->name ?? '—' }} <span class="honorific">御中</span></div>
        <div class="subject-row">
            <span class="subject-label">件名：</span><strong>{{ $sale->subject }}</strong>
        </div>
        <div class="total-box" style="margin-top:14px;">
            <div class="label">ご請求金額（税込）</div>
            <div class="amount">{{ '¥' . number_format($sale->total_amount) }}</div>
        </div>
    </td>
    <!-- 発行情報 -->
    <td class="from-block">
        <table class="meta-table" cellspacing="0" cellpadding="0">
            ...
            <tr>
                <td class="meta-label">納品日</td>
                <td class="meta-value">{{ $sale->delivery_date->format('Y年m月d日') }}</td>
            </tr>
            @endif
            @if($sale->employee)
            <tr>
                <td class="meta-label">担当者</td>
                <td class="meta-value">{{ $sale->employee->name }}</td>
            </tr>
            @endif
        </table>
    </td>
...
<table class="items-table" cellspacing="0" cellpadding="0">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td class="text-right">¥{{ number_format((float)$item->uri_tan) }}</td>
            <td class="text-center">{{ $item->tax_rate }}%</td>
            <td class="text-right">¥{{ number_format((float)$item->sale_amount) }}</td>
...
<table class="summary-table" cellspacing="0" cellpadding="0">
    <tr>
        <td class="s-label">売上小計</td>
        <td class="s-value">¥{{ number_format((float)$sale->subtotal) }}</td>
    </tr>
    ...
    <tr class="total-row">
        ...
        <td class="s-value">¥{{ number_format((float)$sale->total_amount) }}</td>
    </tr>
</table>
