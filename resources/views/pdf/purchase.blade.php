...
<title>仕入書 {{ $purchase->purchase_number }}</title>
<style>
    @font-face {
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .purchase-number { font-size: 13px; font-weight: bold; }
    .total-box {
...
<div class="doc-title">仕　入　書</div>

<table class="header-body" cellspacing="0" cellpadding="0">
<tr>
    <!-- 仕入先 -->
    <td class="to-block">
        <div class="supplier-name">{{ $purchase->supplier?->name ?? '—' }} <span class="honorific">御中</span></div>
        <div class="subject-row">
            <span class="subject-label">件名：</span><strong>{{ $purchase->subject }}</strong>
        </div>
        <div class="total-box" style="margin-top:14px;">
            <div class="label">仕入金額（税込）</div>
            <div class="amount">{{ '¥' . number_format($purchase->total_amount) }}</div>
        </div>
    </td>
    <!-- 発行情報 -->
    <td class="from-block">
        ...
        <table class="meta-table" cellspacing="0" cellpadding="0">
            <tr>
                ...
                <td class="meta-value purchase-number">{{ $purchase->purchase_number }}</td>
            </tr>
            <tr>
                <td class="meta-label">仕入日</td>
                <td class="meta-value">{{ $purchase->purchase_date?->format('Y年m月d日') }}</td>
            </tr>
            @if($purchase->employee)
            <tr>
                <td class="meta-label">担当者</td>
                <td class="meta-value">{{ $purchase->employee->name }}</td>
            </tr>
            @endif
        </table>
    </td>
...
<table class="items-table" cellspacing="0" cellpadding="0">
    ...
    <tbody>
        ...
        @endforeach
        @for($i = count($purchase->items); $i < 5; $i++)
...
<table class="summary-table" cellspacing="0" cellpadding="0">
    <tr>
        <td class="s-label">仕入小計</td>
        <td class="s-value">¥{{ number_format((float)$purchase->subtotal) }}</td>
    </tr>
    ...
    <tr class="total-row">
        ...
        <td class="s-value">¥{{ number_format((float)$purchase->total_amount) }}</td>
    </tr>
</table>
...
<div class="remarks-section">
    ...
    <div class="remarks-body">{{ $purchase->remarks }}</div>
</div>
@endif
