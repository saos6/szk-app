...
class DashboardController extends Controller
{
    public function __invoke(): Response
    {
        ...
        $topProducts = SaleItem::join('sales', 'sale_items.sale_id', '=', 'sales.id')
            ->where('sales.is_deleted', false)
            ->where('sales.sale_date', '>=', $thisYear)
            ->whereNotNull('sale_items.model_name')
            ->where('sale_items.model_name', '!=', '')
            ->groupBy('sale_items.model_name')
            ->orderByDesc('total')
            ->limit(8)
            ->selectRaw('sale_items.model_name as name, coalesce(sum(sale_items.sale_amount), 0) as total')
            ->get()
            ->map(fn ($r) => ['name' => $r->name, 'total' => (float) $r->total])
            ->values();
