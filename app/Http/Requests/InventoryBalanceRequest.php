...
class InventoryBalanceRequest extends FormRequest
{
        ...
        return [
            'stock_ym' => [
                ...
                Rule::unique('inventory_balances', 'stock_ym')
                    ...
                    ->where('model_code', $this->input('model_code'))
                    ->where('frame_number', $this->input('frame_number'))
                    ->where('is_deleted', 0)
                    ->ignore($model?->id),
            ],
            'warehouse_code' => [
            ...
            'model_code' => [
                ...
                Rule::exists('vehicle_models', 'model_code')->where('is_deleted', 0),
            ],
    ...
    public function messages(): array
    {
        return [
            ...
            'model_code.exists'       => '選択された機種コードが存在しません。',
